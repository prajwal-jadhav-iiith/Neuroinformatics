...
channel2use    = 'p7';           % electrode to analyze
frequency2plot = 15;              % in Hz (for single frequency plot)
timewin        = 400;             % window size in ms
times2save     = -300:50:1000;    % time points to save (in ms)
baseline_win   = [-300 -100];     % baseline window in ms
...
timewinidx = round(timewin/(1000/EEG.srate));
chan2useidx = strcmpi(channel2use, {EEG.chanlocs.labels});

% baseline indices
[~,baseline_start] = min(abs(times2save - baseline_win(1)));
[~,baseline_end]   = min(abs(times2save - baseline_win(2)));

nfft = timewinidx;
frex = linspace(0, EEG.srate/2, floor(nfft/2)+1);
...
for timepointi = 1:length(times2save)
    ...
    tempdat = squeeze(EEG.data(chan2useidx, ...
        times2saveidx(timepointi)-floor(timewinidx/2):...
        times2saveidx(timepointi)+floor(timewinidx/2)-mod(timewinidx+1,2), :));
    
    power_trials = zeros(length(frex), EEG.trials);
    
    for triali = 1:EEG.trials
        [pxx, f] = pmtm(tempdat(:, triali), nw, nfft, EEG.srate);        
        power_trials(:, triali) = pxx;
    end
    
    tf(:, timepointi) = mean(power_trials, 2);
end

baseline_power = mean(tf(:, baseline_start:baseline_end), 2);
tf_db = 10*log10(bsxfun(@rdivide, tf, baseline_power));

figure('Position', [100 100 1200 800]);

[~, freq2plotidx] = min(abs(frex - frequency2plot));

subplot(2, 2, 1)
contourf(times2save, frex, log10(tf), 40, 'linecolor', 'none')
xlabel('Time (ms)')
ylabel('Frequency (Hz)')
title(['Multitaper (pmtm) - Sensor ' channel2use ' (No Baseline Correction)'])
colorbar
set(gca, 'ylim', [0 40])
hold on
plot([0 0], get(gca,'ylim'), 'w--', 'LineWidth', 2)

subplot(2, 2, 2)
contourf(times2save, frex, tf_db, 40, 'linecolor', 'none')
xlabel('Time (ms)')
ylabel('Frequency (Hz)')
title(['Multitaper (pmtm) - Sensor ' channel2use ' (Baseline Corrected)'])
colorbar
set(gca, 'clim', [-3 3], 'ylim', [0 40])
hold on
plot([baseline_win(1) baseline_win(1)], get(gca,'ylim'), 'w--', 'LineWidth', 2)
plot([baseline_win(2) baseline_win(2)], get(gca,'ylim'), 'w--', 'LineWidth', 2)
plot([0 0], get(gca,'ylim'), 'w--', 'LineWidth', 2)
