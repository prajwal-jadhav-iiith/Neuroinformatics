...
% load the eeglab data set.
eeglab
EEG = pop_loadset('eeglab_data.set', fullfile(fileparts(which('eeglab')), 'sample_data'));

% extract the data part from the EEG structure
data = EEG.data;       % channels x samples
% extract the channel labels from EEG structure
...
% store desired frequency to filter in wo by dividing 60 with half of sampling frequency
% store the bandwidth of notch filter as a fraction of wo, can be other
% value as well
wo = 60/(EEG.srate/2); bo = wo/35;
% create a notch filter by using matlab provided function
[bn,an] = designNotchPeakIIR(CenterFrequency=wo, bandwidth = bo, Response="notch");
% run the notch filter on our data
data_filt = filtfilt(bn,an,data')';

% -------------------------
% Step 2: Plot PSD before & after filtering
% -------------------------
% the four lines below plot the power spectral density with two different
% data. Firstly, with original EEG data and later with filtered data from
% above steps
figure;
...
% -------------------------
% the four lines below calculate the average/mean of the notch filtered
% data, subtract the mean from the data and then plots the power spectral
% density.
...
% -------------------------
% event latencies and event types are metadata associated with the EEG data
event_latencies = [EEG.event.latency]; 
event_types     = {EEG.event.type};
% find all the indices that have event type as 'rt'
target_idx = find(strcmp(event_types,'rt'));
% the epoch window is pair of indices that start and end an epoch
epoch_window = round([-0.2 0.8]*EEG.srate);  
% length of the epoch window
epoch_len = diff(epoch_window)+1;

% Preallocate
% create epochs/segments from the EEG data and store it in epochs variable
...
% initialize baseline variables for subtractign base line from original
% epochs
baseline_idx = 1:round(0.2*EEG.srate);
% we use the average of the epoch falling in baseline window
...
% extract number of channels and number of trials to use in a spectrogram
% function
...
% for each channel and for each trial calculate the spectrogram and store
% it in a 4D array
for ch = 1:nChans
    for tr = 1:nTrials
